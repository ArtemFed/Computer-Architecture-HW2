...
# ./main.c:14:         time_t t_start = clock();
	call	clock@PLT
	mov	QWORD PTR -32[rbp], rax	# t_start
...
# ./main.c:19:         time_t t_end = clock();
	call	clock@PLT
	mov	QWORD PTR -40[rbp], rax	# t_end
# ./main.c:21:         printf("Time: %d ms", (int) (difftime(t_end, t_start)) / 1000);
	mov	rdx, QWORD PTR -32[rbp]
	mov	rax, QWORD PTR -40[rbp]
	mov	rsi, rdx	# t_start => difftime()
	mov	rdi, rax	# t_end => difftime()
	call	difftime@PLT
	cvttsd2si	eax, xmm0
	movsx	rdx, eax
...
# ./main.c:46:         int length = 0;
	mov	DWORD PTR -48[rbp], 0	# length = 0
...
# ./main.c:53:         printf("\nResult: %d\n", task_random(length));
	...
	mov	edi, eax	# length => task_random(length)
